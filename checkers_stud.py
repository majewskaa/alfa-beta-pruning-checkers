...
from checkers_no_vis import minimax_a_b, ITERATIONS_LIMIT
...
#-----------------------------------------------------------#
    ...
    def make_ai_move(self, move):
        ...
        if self.white_turn and d_row==0:#damka biała
            self.board[d_row][d_col] = King(self.board[d_row][d_col])

        if not self.white_turn and d_row==BOARD_WIDTH-1:#damka niebieska
            self.board[d_row][d_col] = King(self.board[d_row][d_col])
...
def main():
    ...
    while is_running:
        ...
        if iterations >= ITERATIONS_LIMIT/2:
            end = 'tie'
        ...
        if not game.board.white_turn:
            move = minimax_a_b(deepcopy(game.board), MINIMAX_DEPTH, EVALUATE_OPTION)
